...
//       return "No account found with this email or phone number. Please sign up first.";
//     }
...
//     }
//     if (errorCode.includes("auth/phone-number-invalid")) {
//       return "Invalid phone number. Please enter a valid 11-digit number starting with 03.";
//     }
//     if (errorCode.includes("auth/invalid-phone-number")) {
//       return "Invalid phone number format. Please use 03XXXXXXXXX.";
//     }
...
//   // ✅ Handle redirect from Firebase default page
//   useEffect(() => {
//     const mode = new URLSearchParams(window.location.search).get("mode");
//     const apiKey = new URLSearchParams(window.location.search).get("apiKey");
//     const oobCode = new URLSearchParams(window.location.search).get("oobCode");
...
//   // ✅ Check reset flag after user logs in
//   useEffect(() => {
//     const checkResetFlag = async () => {
//       if (user && isRedirectFromReset) {
//         console.log("🔍 Checking reset flag for user:", user.uid);
...
//     } catch (err: any) {
//       // ✅ Use clean error message
//       const cleanErr = getCleanErrorMessage(err);
//       setResetError(cleanErr);
//     } finally {
//       setResetLoading(false);
//     }
//   };
...
//       console.error("Update password error:", err);
//       // ✅ Use clean error message
//       setUpdateError(getCleanErrorMessage(err));
//     } finally {
//       setUpdateLoading(false);
//     }
//   };
...
export default function LoginPage() {
    ...
    if (user) {
      const checkRTDB = async () => {
        try {
          ...
          if (typeof checkAndUpdateRTDBPassword === 'function') {
            const needsUpdate = await checkAndUpdateRTDBPassword(user);
            if (needsUpdate) {
              setShowPasswordUpdateModal(true);
              setIsCheckingAuth(false);
  ...
  }, [user, router, checkAndUpdateRTDBPassword]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isPhone) {
        ...
        await loginWithEmail(cleanEmailOrPhone, formData.password);
      } else {
        setError("Please enter a valid email or 11-digit phone number");
        setLoading(false);
        return;
...
Please check the user's identity and reset their password from the admin panel.`;
  ...
  // If user is logged in and no password update needed, don't render
  if (user && !showPasswordUpdateModal) {
    return null;
  }
