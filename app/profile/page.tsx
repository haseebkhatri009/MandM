...
//               animate={{ scale: 1, y: 0 }}
//               exit={{ scale: 0.9, y: 20 }}
//               className="bg-card rounded-lg shadow-xl p-6 max-w-md w-full border border-border max-h-[90vh] overflow-y-auto"
//               onClick={(e) => e.stopPropagation()}
//             >
...
//update change password in rtdb
...
export default function ProfilePage() {
  ...
  // ✅ UPDATE PASSWORD IN RTDB
  const updatePasswordInRTDB = async (uid: string, newPassword: string) => {
    try {
      const db = getDatabase();
      ...
      if (snapshot.exists()) {
        ...
        console.log('✅ Password updated in RTDB');
      } else {
        // User doesn't exist - Create full user object
        await set(userRef, {
          uid: uid,
          email: user?.email || '',
        ...
        console.log('✅ New user created in RTDB with password');
      }
    } catch (error) {
      console.error('Error updating password in RTDB:', error);
      throw error;
    }
  };
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Step 3: Update password in RTDB
      await updatePasswordInRTDB(user.uid, newPassword);
      ...
      if (err.code === "auth/wrong-password") {
        ...
        showCustomToast("❌ Old Password is incorrect", "error");
      }
    } finally {
      setPasswordLoading(false);
    }
