...
//   // ✅ Info Toast - 1 SECOND DURATION
//   const showInfoToast = (message: string, icon: string = 'ℹ️') => {
//     toast.custom(
//       (t) => (
//         <div
//           className={`${
...
//   const handleUnauthorizedAction = () => {
//     if (!user) {
//       showErrorToast('Please login to manage your cart', '🔒');
//       setTimeout(() => {
//         router.push('/login?from=cart');
//       }, 1500);
//       return true;
...
//     removeFromCart(item.id);
//     showCustomToast('Item removed from cart', '🗑️');
//   };
...
//     clearCart();
//     showCustomToast('Cart cleared successfully', '🧹');
//   };
...
//         const totalQty = selectedFlavors.reduce((sum, f) => sum + f.quantity, 0);
//         showCustomToast(`${selectedFlavors.length} flavors selected (${totalQty} units)`, '✅');
//       }
//     } catch (error) {
//       console.error('Error saving flavor selections:', error);
//       if (error instanceof Error && error.message.includes('Permission denied')) {
//         showErrorToast('Permission denied - please check Firebase rules', '🔒');
//       } else {
//         showErrorToast('Failed to save flavors', '❌');
//       }
//     }
...
//     if (Object.keys(stockErrors).length > 0) {
//       showErrorToast('Please fix stock issues before proceeding to checkout', '⚠️');
//       return;
//     }
...
//     if (unselectedFlavors.length > 0) {
//       showErrorToast(`Please select flavors for: ${unselectedFlavors.join(', ')}`, '⚠️');
//       return;
//     }
...
//   // ✅ Check auth - but don't redirect, just show cart
//   useEffect(() => {
//     if (authLoading) return;
//     setIsCheckingAuth(false);
//   }, [authLoading]);
...
//   // ✅ Open flavor selection modal (accessible to all)
//   const openFlavorModal = (productId: string) => {
//     const product = productDetails[productId];
//     if (!product || !product.hasFlavors || !product.flavors) return;
...
//   // ✅ Update flavor quantity in temp
//   const updateFlavorQuantityInTemp = (flavorName: string, newQuantity: number) => {
//     if (!tempFlavorSelection) return;
//     const newSelectedFlavors = tempFlavorSelection.selectedFlavors.map(f => {
//       if (f.flavorName === flavorName) {
...
//   // ✅ Confirm flavor selection (saves to RTDB only if logged in)
//   const confirmFlavorSelection = async () => {
//     if (!tempFlavorSelection || !selectedProductId) return;
...
//     try {
//       // ✅ If user is logged in, save to RTDB
//       if (user) {
//         const cartSelectionsRef = ref(rtdb, `cart_selections/${user.uid}`);
...
//     } catch (error) {
//       console.error('Error saving flavor selections:', error);
//       showErrorToast('Failed to save flavors locally', '❌');
//     }
...
//   // ✅ Handle Checkout - Always enabled
//   const handleCheckout = () => {
//     if (Object.keys(stockErrors).length > 0) {
//       showErrorToast('Please fix stock issues before proceeding to checkout', '⚠️');
//       return;
...
//   // ✅ Show empty cart state - Always accessible
//   if (cartItems.length === 0) {
//     return (
//       <div className="min-h-screen bg-background">
//         <Navbar />
...
//       {/* ✅ Flavor Selection Modal - Accessible to all */}
//       <AnimatePresence>
//         {isFlavorModalOpen && selectedProductId && tempFlavorSelection && (
//           <>
//             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} exit={{ opacity: 0 }} onClick={() => setIsFlavorModalOpen(false)} className="fixed inset-0 bg-black z-40" />
...
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import { Trash2, Plus, Minus, ArrowLeft, ShoppingCart, AlertCircle, Package, X, Check, Truck, RotateCcw, ChevronDown } from 'lucide-react';
import { useEffect, useState } from 'react';
import Navbar from '@/components/Navbar';
import { useCart } from '@/lib/cartContext';
import { useAuth } from '@/lib/authContext';
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="py-12 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          {/* ✅ Shipping & Return Policy Button */}
          <div className="mb-6">
            <button
              onClick={() => setIsPolicyOpen(!isPolicyOpen)}
              className="flex items-center gap-3 px-5 py-3 bg-secondary/80 hover:bg-secondary rounded-xl border border-border transition-all w-full sm:w-auto"
            >
              <div className="flex items-center gap-2">
                <Truck className="w-5 h-5 text-primary" />
                <RotateCcw className="w-5 h-5 text-accent" />
                <span className="font-semibold text-foreground">Shipping &amp; Return Policy</span>
              </div>
              <ChevronDown className={`w-5 h-5 text-muted-foreground transition-transform ${isPolicyOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            <AnimatePresence>
              {isPolicyOpen && (
                ...
                >
                  <div className="mt-3 p-6 bg-card rounded-xl border border-border shadow-lg">
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                          <p>📦 <strong>Delivery Time:</strong> Orders are typically delivered within 2-4 business days after order confirmation.</p>
                          <p>📍 <strong>Delivery Areas:</strong> We deliver across all major cities in Pakistan.</p>
                          <p>💰 <strong>Delivery Charges:</strong> {formatPrice(deliveryCharges)} delivery charges apply on all orders (for Karachi).</p>
                          <p>🎉 <strong>Free Delivery:</strong> Orders above PKR {formatPrice(minOrderForFreeDelivery)} qualify for free delivery.</p>
                          <p>⏰ <strong>Order Tracking:</strong> You will receive a tracking link via WhatsApp/Email once your order is shipped.</p>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                          <p>🔄 <strong>Return Window:</strong> You can return items within 7 days of delivery.</p>
                          <p>📋 <strong>Conditions:</strong> Items must be unused, unopened, and in their original packaging.</p>
                          <p>🚫 <strong>Non-Returnable:</strong> Custom orders, opened products, and items on sale are non-returnable.</p>
                          <p>💰 <strong>Refund Process:</strong> Refunds are processed within 5-7 business days after we receive the returned item.</p>
                          <p>📞 <strong>Contact:</strong> For returns, please contact us via WhatsApp or Email with your order details.</p>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          For any questions regarding shipping or returns, feel free to reach out to us on 
                          <Link href="/contact" className="text-primary hover:underline font-medium ml-1">Contact Page </Link> 
