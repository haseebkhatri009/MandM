...
//   // ✅ FIXED: WhatsApp URL with proper null/undefined check
//   const getWhatsAppUrl = (phoneNumber: string | undefined, orderID: string) => {
//     // ✅ Check if phoneNumber exists and is valid
//     if (!phoneNumber || phoneNumber === 'undefined' || phoneNumber === 'null' || phoneNumber === '') {
//       // ✅ Return default admin number
...
//     // ✅ Check if cleaned number is valid
...
//                     {/* Action Buttons */}
//                     <div className="flex gap-2 flex-wrap">
//                       {/* ✅ FIXED: WhatsApp button with null check */}
//                       <a
//                         href={getWhatsAppUrl(order.phoneNumber, order.id)}
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         className="inline-flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:opacity-90 transition-opacity font-semibold text-sm"
//                       >
...
//                 <h3 className="font-semibold mb-2">Customer Information</h3>
//                 <p className="text-sm text-muted-foreground"><strong>Name:</strong> {selectedOrder.customerName}</p>
//                 <p className="text-sm text-muted-foreground"><strong>Phone:</strong> {selectedOrder.phoneNumber || 'N/A'}</p>
//                 <p className="text-sm text-muted-foreground"><strong>Email:</strong> {selectedOrder.email}</p>
//                 <p className="text-sm text-muted-foreground"><strong>Address:</strong> {selectedOrder.address}, {selectedOrder.city}</p>
//               </div>
...
export default function AdminOrdersPage() {
  ...
  // ✅ Get display name for item with flavors
                              ...
                              <div key={idx} className="flex justify-between items-start">
                                <div>
                                  ...
                                  <div className="text-xs text-muted-foreground">
                                    ×{qty} @ {formatPrice(item.price - (item.discount || 0))}
                                  </div>
                                </div>
                          ...
                          {order.items.map((item, idx) => {
                            ...
                            return (
                              <div key={idx} className="flex flex-col text-sm border-b border-gray-200 pb-2">
                                ...
                                <div className="flex justify-between text-xs text-muted-foreground">
                                  <span>
                                    Unit Price: {formatPrice(finalPrice)} × {qty}
                                  </span>
                                  {item.discount && item.discount > 0 && (
                                    <span className="text-green-600">
                                      💰 Saved: {formatPrice(item.discount)} each
                                ...
                                {item.isFlavorProduct && item.selectedFlavors && item.selectedFlavors.length > 0 && (
                                  <div className="text-xs text-green-600 mt-1">
                                    Flavors: {item.selectedFlavors.map(f => `${f.flavorName} (×${f.quantity})`).join(', ')}
                                  </div>
                                )}
                              </div>
                            );
                  ...
                  {selectedOrder.items.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex flex-col text-sm p-2 bg-secondary rounded">
                        ...
                        <div className="flex justify-between text-xs text-muted-foreground">
                          <span>Unit: {formatPrice(item.price - (item.discount || 0))} × {qty}</span>
                          {item.discount && item.discount > 0 && (
                            <span className="text-green-600">Saved: {formatPrice(item.discount)} each</span>
                          )}
                        </div>
