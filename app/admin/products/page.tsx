...
//with deal bg without edit btn scroll to top
...
//ok without items include in deal
...
// import { ref, onValue, set, remove } from 'firebase/database';
// import { uploadToImgBB } from '@/lib/imgbb';
// import { Upload, Trash2, Edit2, ArrowLeft, X, Star, Plus, Tags, Package, Tag, Palette, Sparkles, List, Box, Info } from 'lucide-react';
// import Link from 'next/link';
// import toast, { Toaster } from 'react-hot-toast';
...
//   // ✅ Deal Items functions
//   const addDealItem = () => {
//     setFormData(prev => ({
//       ...prev,
//       dealItems: [...prev.dealItems, '']
//     }));
//   };

//   const removeDealItem = (index: number) => {
//     const newItems = [...formData.dealItems];
...
//   const updateDealItem = (index: number, value: string) => {
//     const newItems = [...formData.dealItems];
...
//       const flavorNames = formData.flavors.filter(f => f.trim() !== '');
//       const waxVariantNames = formData.waxVariants.filter(v => v.trim() !== '');
//       const dealItemNames = formData.dealItems.filter(item => item.trim() !== '');
...
//           productData.dealColor = formData.dealColor;
//         }
//         if (dealItemNames.length > 0) {
//           productData.dealItems = dealItemNames;
//         }
//         // ✅ Deal doesn't have flavors
//         productData.hasFlavors = false;
//       } else {
//         // ✅ Only non-deal products can have flavors
//         if (hasFlavors && flavorNames.length > 0) {
//           productData.hasFlavors = true;
//           productData.flavors = flavorNames;
//         }
...
//                     {/* ✅ Deal Items Included Section */}
//                     <div className="md:col-span-2">
//                       <div className="space-y-4 p-4 bg-blue-50/50 rounded-lg border border-blue-200">
//                         <div className="flex items-center justify-between">
//                           <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
//                             <Box className="w-5 h-5 text-blue-600" />
//                             Items Included in Deal
//                           </h3>
//                           <button
//                             type="button"
//                             onClick={addDealItem}
...
//                         <p className="text-xs text-muted-foreground">
//                           📦 List all items that are included in this deal. Customers will see these items.
//                         </p>
...
//                               animate={{ opacity: 1, y: 0 }}
//                               exit={{ opacity: 0, y: -10 }}
//                               className="flex items-center gap-3 bg-white p-3 rounded-lg border border-blue-200 shadow-sm"
//                             >
//                               <span className="text-sm font-semibold text-gray-500 w-8">#{index + 1}</span>
//                               <input
//                                 type="text"
//                                 value={item}
//                                 onChange={(e) => updateDealItem(index, e.target.value)}
...
//                           <p className="text-center text-gray-500 py-4">No items added yet. Click "Add Item" to add one.</p>
//                         )}

//                         {/* ✅ Summary of Deal Items */}
//                         {formData.dealItems.filter(item => item.trim() !== '').length > 0 && (
...
//                               {formData.dealItems.filter(item => item.trim() !== '').map((item, i) => (
...
//                 {/* ✅ Only show "has multiple variants" for non-deal products */}
//                 {formData.category !== 'Deal' && (
//                   <div className="md:col-span-2">
//                     <div className="flex flex-wrap items-center gap-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
//                       <label className="flex items-center cursor-pointer">
...
//                         Deals do not support multiple variants. Use "Items Included" section above to list deal contents.
//                       </p>
//                     </div>
//                   </div>
//                 )}
...
//                   {product.dealItems && product.dealItems.length > 0 && (
//                     <div className="absolute bottom-2 left-2 bg-green-500/80 text-white text-xs px-2 py-1 rounded flex items-center gap-1">
//                       <List className="w-3 h-3" />
//                       {product.dealItems.length} Items
//                     </div>
//                   )}
...
//                   {/* ✅ Show Deal Items if available */}
//                   {product.category === 'Deal' && product.dealItems && product.dealItems.length > 0 && (
//                     <div className="mb-3 sm:mb-4 p-2 bg-green-50 rounded-lg border border-green-200">
//                       <p className="text-xs font-semibold text-green-800 flex items-center gap-1">
//                         <List className="w-3 h-3" />
//                         Items Included:
//                       </p>
//                       <ul className="mt-1 space-y-0.5">
//                         {product.dealItems.filter(item => item.trim() !== '').map((item, i) => (
